...
class SEANet(nn.Module):
    def __init__(self, num_classes, vgg16_caffe_path=None, pretrained=False):
        ...
        #attention moduel
        self.avgpool = nn.AvgPool2d(kernel_size=3, stride=2, padding=1, ceil_mode=True)
        self.aconv = nn.Conv2d(1024, 1, kernel_size=1, stride=1, bias=False)
    ...
    def forward(self, x):
        ...
        mask = self.aconv(mask)
        mask = self.sigmoid(mask)

        x = self.classifier((1+mask)*x)
        return mask,x
